...
export class Tab4Page implements OnInit {
  ...
  getComp(){
    const queryObservable = this.orders.pipe(
      switchMap(() => 
        this.comp = this.afs.collection('orders', ref => ref.where('user_id', '==', this.authService.userData.uid).where('order_status', '==', 'Completed')).valueChanges()
    ...
    // subscribe to changes
    queryObservable.subscribe(data => {
    console.log("to complete items:", data);  
